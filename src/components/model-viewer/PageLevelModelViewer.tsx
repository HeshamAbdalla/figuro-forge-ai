...
import { X, Maximize2, Minimize2, RotateCcw, Download, Info, Camera, Settings, Loader2, ArrowLeft } from "lucide-react";
...
interface PageLevelModelViewerProps {
  modelUrl: string | null;
  fileName?: string;
  onBack: () => void;
}

const PageLevelModelViewer: React.FC<PageLevelModelViewerProps> = ({
  modelUrl,
  fileName,
  onBack
}) => {
  ...
  useEffect(() => {
    if (modelUrl && modelUrl !== stableModelUrl) {
      console.log('PageLevelModelViewer: Setting model URL:', modelUrl);
      setIsLoading(true);
      setModelError(null);
  ...
  const handleModelError = (error: any) => {
    console.error("PageLevelModelViewer: Model loading error:", error);
  ...
  return (
    <div className={cn(
      "min-h-screen bg-figuro-dark transition-all duration-500",
      ...
      <div className={cn(
        ...
        isFullscreen ? "h-[calc(100vh-120px)]" : "h-[calc(100vh-180px)]"
      )}>
        {isLoading ? (
          <motion.div
            initial={{ opacity: 0 }}
...
export default PageLevelModelViewer;
